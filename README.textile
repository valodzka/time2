...
Also it has pure ruby implementation of time parsing method, which is "awfully slow":http://scie.nti.st/2007/11/28/ruby-vs-python-date-parsing-am-i-doing-somethin-wrong. 

Project tries to radically solve this problems. First of all, it adds full platform independent time zone support (including full tz database, transition between time zones, dst) and fast implementation of the <code>strptime</code> method. 
...
Code tested on Debian Etch with GCC, should also work on Windows with Visual C++ 2008 (small modifications might be required) 
