...
About 13 times faster than TZInfo, but can be event faster if use <code>local</code> instead of  <code>getlocal</code> which creates new object every time
...
About 50 times faster than <code>strptime</code> from ruby standard library (we use <code>Method#call</code> to avoid redefinition)
...
Code tested on Debian Lenny with GCC, should also work on Windows with Visual C++ 2008 (small modifications might be required)
...
Same terms as Ruby unless otherwise declared in specific file
