...
Ruby Time class is one of the most disappointment part of ruby standart library. It "can't handle time zones":http://michaelbuffington.com/2006/10/11/i-heap-scorn-upon-rubys-time-class/ by desing (but has methods which accept timezone as parameter) and this can't be changed by "third party libraries":http://tzinfo.rubyforge.org (except as through creation child class (as "Rails does":http://api.rubyonrails.org/classes/ActiveSupport/TimeWithZone.html)). It has pure ruby implementation of time parsing function, but it "awful slow":http://scie.nti.st/2007/11/28/ruby-vs-python-date-parsing-am-i-doing-something-wrong.

Project attempts radically solve this problems. It updates core ruby Time class. Based on code from trunk, it, first of all, adds full platform independent time zones support, including full tz database, transition between time zones, dst, and secondary it add fast platform independent implementation of <code>strptime</code>. 

Library will be distributed as a gem when it became production ready, but I hope this code will be included in ruby core.
...
About 50 times faster then strptime from ruby standart library (we use <code>method#call</code> to avoid redefinition)
...
* <code>strptime</code> implemenation is very raw
* code isn't clean enough, many old code should be removed
* ugly english :)

h3. Acknowledgements

* "tz database":http://www.twinsun.com/tz/tz-link.htm for the exelent "time zones database and inial version of time functions":ftp://elsie.nci.nih.gov/pub
