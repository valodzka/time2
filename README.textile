...
* It has a couple of "non-portable features":http://www.ruby-doc.org/core/classes/Time.src/M000250.html, because havy depend from underluing system libraries. On Windows, for example, you can't create time before UNIX epoch (i.e. 1970).
...
Code tested on Debian Etch with GCC, should also work on Windows with VC 9 (small modifications might be required) 

h3. TODO

* currently issue segfault with methods which uses internal Time API, i.e. some <code>File</code>, <code>Socket</code> and <code>Zlib</code> methods
* <code>strptime</code> implementation doesn't support %Q, %N and some other ruby specific flags
